...
#define SHOW_HACK_MESSAGE user.visible_message(SPAN_DANGER("[user]骇入了[target]。"), SPAN_NOTICE("You hack [target]."))
...
/obj/item/device/multitool/antag/afterattack(atom/target, mob/user, flag)
	...
	if(istype(target, /obj/structure/machinery/door/airlock))
		...
		user.visible_message(SPAN_DANGER("[user]开始骇开[target]！"), SPAN_NOTICE("You start to hack open [target]."))

		if(!do_after(user, hack_speed, INTERRUPT_ALL, BUSY_ICON_HOSTILE, target, INTERRUPT_ALL))
			to_chat(user, SPAN_WARNING("你决定不骇入[target]。"))
			return

		user.visible_message(SPAN_DANGER("[user]骇开了[target]。"), SPAN_NOTICE("You hack open [target]."))

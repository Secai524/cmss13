...
/mob/living/carbon/xenomorph/verb/purchase_strain()
	...
	var/strain_choice = tgui_input_list(usr, "你想选择哪个变种？", "Choose Strain", strain_list, theme = "hive_status")
	if(!strain_choice)
		return
...
/mob/living/carbon/xenomorph/verb/reset_strain()
	...
	if(!COOLDOWN_FINISHED(src, next_strain_reset))
		to_chat(src, SPAN_WARNING("我们力量不足，无法重置变种。我们将在[round((next_strain_reset - world.time) / 600, 1)]分钟后能够重置。"))
		return

	// Show the user the strain's description, and double check that they want it.
	if(tgui_alert(src, "你确定吗？", "重置变种", list("Yes", "No")) != "Yes")
		return
...
/mob/living/carbon/xenomorph/proc/can_take_strain(reset=FALSE)
	...
	if(strain && !reset)
		to_chat(src, SPAN_WARNING("我们已经选择了一个变种。"))
		return FALSE

	if(!strain && reset)
		to_chat(src, SPAN_WARNING("重置前必须先选择一个菌株。"))
		return FALSE

	if(is_zoomed)
		to_chat(src, SPAN_WARNING("我们无法在远眺时进行此操作。"))
		return FALSE

	if(is_ventcrawling)
		to_chat(src, SPAN_WARNING("此地过于狭窄，无法容纳菌株。"))
		return FALSE

	if(!isturf(loc))
		to_chat(src, SPAN_WARNING("我们无法在此处容纳菌株。"))
		return FALSE

	if(handcuffed || legcuffed)
		to_chat(src, SPAN_WARNING("束缚装置限制过严，使我们无法容纳菌株。"))
		return FALSE

	if(health < maxHealth)
		to_chat(src, SPAN_WARNING("我们必须处于完全健康状态才能容纳菌株。"))
		return FALSE

	if(agility || fortify || crest_defense || stealth)
		to_chat(src, SPAN_WARNING("我们无法在此姿态下容纳菌株。"))
		return FALSE

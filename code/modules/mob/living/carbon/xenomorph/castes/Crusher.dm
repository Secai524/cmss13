...
/mob/living/carbon/xenomorph/proc/handle_collision(atom/target)
	...
	//Barricade collision
		...
		visible_message(SPAN_DANGER("[src]猛撞上[blockade_in_path]，滑行着停了下来！"), SPAN_XENOWARNING("We ram into [blockade_in_path] and skid to a halt!"))
		...
		visible_message(SPAN_DANGER("[src]猛撞上[vehicle_in_path]，滑行着停了下来！"), SPAN_XENOWARNING("We ram into [vehicle_in_path] and skid to a halt!"))
		...
		visible_message(SPAN_DANGER("[src]猛撞[weapon_in_path]！"), SPAN_XENODANGER("We ram [weapon_in_path]!"))
		...
		visible_message(SPAN_DANGER("[src]猛撞[defenses_in_path]！"), SPAN_XENODANGER("We ram [defenses_in_path]!"))
		...
		if (vending_in_path.unslashable)
			...
			visible_message(SPAN_DANGER("[src]径直撞进了[vending_in_path]！"), SPAN_XENODANGER("We smash straight into [vending_in_path]!"))
			playsound(loc, "punch", 25, 1)
		...
		if (fence.cut)
			...
			src.visible_message(SPAN_DANGER("[src]撞上了[fence]！"))
			fence.cut_grille()
			. = TRUE
	...
	// Anything else?
		...
		if (isobj(target))
			...
			if (objects_in_path.unacidable)
				...
				visible_message(SPAN_DANGER("[src]碾碎了[objects_in_path]！"), SPAN_XENODANGER("We crush [objects_in_path]!"))
				...
				visible_message(SPAN_WARNING("[src]将[objects_in_path]撞到一边！"), SPAN_XENOWARNING("We knock [objects_in_path] aside.")) //Canisters, crates etc. go flying.
				playsound(loc, "punch", 25, 1)
...
/datum/behavior_delegate/crusher_base/melee_attack_additional_effects_target(mob/living/carbon/target)
	...
	for (var/mob/living/carbon/aoe_targets in orange(1, target))
		...
		to_chat(aoe_targets, SPAN_XENODANGER("[bound_xeno]劈砍[aoe_targets]！"))
		to_chat(bound_xeno, SPAN_XENODANGER("我们劈砍[aoe_targets]！"))
...
/datum/action/xeno_action/activable/pounce/crusher_charge/additional_effects_always()
	...
	for (var/mob/living/carbon/target in orange(1, get_turf(xeno)))
		...
		to_chat(target, SPAN_XENODANGER("你被[xeno]撞击地面的震动所减速！"))
...
/datum/action/xeno_action/activable/pounce/crusher_charge/additional_effects(mob/living/target)
	...
	xeno.visible_message(SPAN_XENODANGER("[xeno]冲垮了[target_to_check]，将其残忍地踩在脚下！"), SPAN_XENODANGER("We brutalize [target_to_check] as we crush them underfoot!"))
...
/datum/action/xeno_action/onclick/crusher_stomp/use_ability(atom/Atom)
	...
	xeno.visible_message(SPAN_XENODANGER("[xeno]猛击地面！"), SPAN_XENODANGER("We smash into the ground!"))
	xeno.create_stomp()
	...
	for (var/mob/living/carbon/targets in orange(distance, xeno))
		...
		targets.apply_effect(get_xeno_stun_duration(targets, 0.2), WEAKEN)
		to_chat(targets, SPAN_XENOHIGHDANGER("你被[xeno]撞得失去平衡，速度减缓！"))
...
/datum/action/xeno_action/onclick/crusher_stomp/charger/use_ability()
	...
	for (var/mob/living/carbon/target_to_stomp in get_turf(xeno)) // MOBS ONTOP
		...
		to_chat(target_to_stomp, SPAN_XENOHIGHDANGER("你被[xeno]残忍地碾压并践踏！！！"))
	...
	for (var/mob/living/carbon/targets_to_get in orange(distance, get_turf(xeno))) // MOBS AROUND
		...
		if(stomped_carbon)
			to_chat(targets_to_get, SPAN_XENOHIGHDANGER("你看着[stomped_carbon]被[xeno]碾碎！"))
		to_chat(targets_to_get, SPAN_XENOHIGHDANGER("[xeno]撼动大地，令你站立不稳！"))
...
/datum/action/xeno_action/onclick/crusher_shield/use_ability(atom/Target)
	...
	xeno.visible_message(SPAN_XENOWARNING("[xeno]蹲伏下来，巩固了防御！"), SPAN_XENOHIGHDANGER("We hunker down and bolster our defenses!"))
...
/datum/action/xeno_action/onclick/crusher_shield/proc/remove_explosion_immunity()
	...
	xeno.explosivearmor_modifier -= 1000
	xeno.recalculate_armor()
	to_chat(xeno, SPAN_XENODANGER("我们对爆炸伤害的免疫结束了！"))
...
/datum/action/xeno_action/onclick/crusher_shield/proc/remove_shield()
	...
	if (istype(found))
		found.on_removal()
		qdel(found)
		to_chat(xeno, SPAN_XENOHIGHDANGER("我们感到增强的护盾结束了！"))

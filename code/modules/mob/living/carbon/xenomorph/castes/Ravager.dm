...
/datum/behavior_delegate/ravager_base/on_life()
	...
	if (rav_shield && ((rav_shield.last_damage_taken + shield_decay_time) < world.time))
		...
		to_chat(bound_xeno, SPAN_XENODANGER("我们感到护盾正在衰减！"))
...
/datum/action/xeno_action/onclick/empower/use_ability(atom/target)
	...
	if(!activated_once)
		...
		xeno.visible_message(SPAN_XENODANGER("[xeno]开始充能！"), SPAN_XENODANGER("We start empowering ourself!"))
		activated_once = TRUE
...
/datum/action/xeno_action/onclick/empower/proc/actual_empower(mob/living/carbon/xenomorph/xeno)
	...
	xeno.visible_message(SPAN_XENOWARNING("[xeno]被周围的敌人充能了！"), SPAN_XENOWARNING("We feel a rush of power from the surrounding enemies!"))
	xeno.create_empower()
...
/datum/action/xeno_action/onclick/empower/proc/super_empower(mob/living/carbon/xenomorph/xeno, empower_targets, datum/behavior_delegate/ravager_base/behavior)
	xeno.visible_message(SPAN_DANGER("[xeno]发出诡异的红光，借助[empower_targets]个敌对目标的能量进一步充能！"), SPAN_XENOHIGHDANGER("We begin to glow an eerie red, empowered by the [empower_targets] enemies!"))
	xeno.emote("roar")
...
/datum/action/xeno_action/onclick/empower/proc/remove_superbuff(mob/living/carbon/xenomorph/xeno, datum/behavior_delegate/ravager_base/behavior)
	...
	xeno.visible_message(SPAN_DANGER("[xeno]的光芒逐渐暗淡。"), SPAN_XENOHIGHDANGER("Our glow fades away, the power leaving our form!"))
	xeno.remove_filter("empower_rage")
...
/datum/action/xeno_action/activable/pounce/charge/additional_effects(mob/living/living)
	...
	xeno.visible_message(SPAN_XENODANGER("[xeno]在冲锋时用护盾猛击[human]！"), SPAN_XENODANGER("We use our shield to bash [human] as we charge at them!"))
...
/datum/action/xeno_action/activable/scissor_cut/use_ability(atom/target_atom)
	...
	ravager_user.visible_message(SPAN_XENODANGER("[ravager_user]用利爪扫过面前区域！"), SPAN_XENODANGER("We sweep our claws through the area in front of us!"))

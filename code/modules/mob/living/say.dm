	...
	if(ishuman(src))
		if(message_mode) // we are talking into a radio
			if(message_mode == "headset") // default value, means general
				message_mode = "将军"
			log_say("[name != "未知" ? name : "([real_name])"] \[[message_mode]\]: [message] (CKEY: [key]) (JOB: [job]) (AREA: [get_area_name(loc)])")
		else // we talk normally
			log_say("[name != "未知" ? name : "([real_name])"]: [message] (CKEY: [key]) (JOB: [job]) (AREA: [get_area_name(loc)])")
	else
		log_say("[name != "未知" ? name : "([real_name])"]: [message] (CKEY: [key]) (AREA: [get_area_name(loc)])")

...
/proc/shakeship(sstrength, stime, drop, osound = TRUE)
	for(var/mob/living/carbon/current_mob in GLOB.living_mob_list)
		...
		if(sstrength > 7)
			...
			to_chat(current_mob, SPAN_HIGHDANGER("你被猛烈地抛起，然后重重地摔在甲板上！！"))
			if(current_mob.client && osound)

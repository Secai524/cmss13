...
/obj/item/paper/verb/rename()
	...
	var/n_name = strip_html(input(usr, "你想在纸上标注什么？", "Paper Labelling", null)  as text)
...
/obj/item/paper/attack(mob/living/carbon/human/M, mob/living/carbon/user)
	...
	if(user.zone_selected == "eyes")
		...
		user.visible_message(SPAN_NOTICE("你将纸展示给[M]看。"),
		...
		if(ishuman(M))
			...
			if(H == user)
				to_chat(user, SPAN_NOTICE("你用[src]擦掉了脸上的油彩。"))
				H.lip_style = null
				H.update_body()
			else
				user.visible_message(SPAN_WARNING("[user]开始用\the [src]擦掉[H]脸上的油彩。"),
				...
				if(do_after(user, 10, INTERRUPT_ALL, BUSY_ICON_FRIENDLY) && do_after(H, 10, INTERRUPT_ALL, BUSY_ICON_GENERIC)) //user needs to keep their active hand, H does not.
					user.visible_message(SPAN_NOTICE("[user]用\the [src]擦掉了[H]脸上的油彩。"),
...
/obj/item/paper/proc/parsepencode(paper_text, obj/item/tool/pen/P, mob/user as mob, iscrayon = 0)
	...
	paper_text = replacetext(paper_text, "\[rank\]", "[user ? user.get_paygrade(0) : "无"]")
	paper_text = replacetext(paper_text, "\[job\]", "[user ? user.job : "无"]")
	paper_text = replacetext(paper_text, "\[op\]", "[GLOB.round_statistics ? GLOB.round_statistics.round_name : "无"]")
	...
	if(P.heat_source >= 400 && !user.is_mob_restrained())
		...
		spawn(20)
			if(get_dist(src, user) < 2 && user.get_active_hand() == P && P.heat_source)
				user.visible_message("[class][user]将\the [src]彻底烧成了灰烬。灰烬在空中飘散，最后落在地上堆成一团。",
...
/obj/item/paper/verb/seal_paper()
	...
	if(tgui_alert(usr, "你确定要密封这份文件吗？注意：这将禁止修改文件上的任何字段，页尾除外。", "确认", list("Yes", "No"), 20 SECONDS) != "Yes")
		return FALSE
...
/obj/item/paper/Topic(href, href_list)
	...
	if(usr.client.prefs.muted & MUTE_IC)
		to_chat(usr, SPAN_DANGER("你无法在纸上写字（已禁言）。"))
		return

	if(href_list["write"])
		var/id = href_list["write"]
		var/t =  stripped_multiline_input(usr, "输入你想写的内容：", "Write", "", MAX_MESSAGE_LEN)
...
/obj/item/paper/attackby(obj/item/P, mob/user)
	...
	if(istype(P, /obj/item/paper) || istype(P, /obj/item/photo))
		...
		user.drop_inv_item_on_ground(src)
		to_chat(user, SPAN_NOTICE("你将[P.name]夹到了[(src.name == "paper") ? "the paper" : src.name]."))
		...
		if(HAS_TRAIT(P, TRAIT_TOOL_PEN))
			...
			if(!p.on)
				to_chat(user, SPAN_NOTICE("你的笔没开！"))
				return
		...
		to_chat(user, SPAN_NOTICE("你用橡皮图章在纸上盖了章。"))
...
/obj/item/paper/flag
	name = "纸旗"
	desc = "有人粗糙地把一张纸粘在了一根棍子上。你感觉想像个傻瓜一样挥舞它。"
...
/obj/item/paper/research_notes/proc/generate()
	...
	switch(note_type)
		if("synthesis")
			...
			if(!contract)
				name = "[chemical_to_generate.name]合成报告"
			else
				name = "[chemical_to_generate.name]合同"
...
/obj/item/paper/colonial_grunts
	...
	name = "殖民地太空大头兵"
	desc = "一款基于USCM的桌面游戏，易于上手，玩法简单，最重要的是能让整个小队都乐在其中。"
...
/obj/item/paper/captain_brief
	name = "机密行动简报"
	desc = "一份来自USCM最高指挥部关于本舰正在响应的殖民地的机密文件。"
	icon_state = "paper_uscm_words"

...
CLIENT_VERB(cmd_view_custom_event)
	set category = "OOC"
	...
	if(mob && !isobserver(mob) && !isnewplayer(mob))
		...
		if(isxeno(mob))
			...
			if(!X.hive || !GLOB.custom_event_info_list[X.hive.name])
				...
				category = tgui_input_list(src, "选择类别。", "Custom Event Info", list("Global", X.hive.name))
			...
			category = tgui_input_list(src, "选择类别。", "Custom Event Info", list("Global", mob.faction))
			CEI = GLOB.custom_event_info_list[category]

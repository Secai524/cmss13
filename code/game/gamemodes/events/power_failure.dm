...
/proc/power_failure(announce = 1)
	...
	sleep(100)
	if(announce)
		marine_announcement("检测到舰船电力系统异常活动。作为预防措施，电力将被无限期关闭。", "Critical Power Failure", 'sound/AI/poweroff.ogg')
...
/proc/power_restore(announce = 1)
	...
	sleep(100)
	if(announce)
		marine_announcement("电力已恢复。原因：未知。", "Power Systems Nominal", 'sound/AI/poweron.ogg')
...
/proc/power_restore_ship_reactors(announce = TRUE)
	...
	if(announce)
		addtimer(CALLBACK(GLOBAL_PROC, GLOBAL_PROC_REF(marine_announcement), "电力已恢复。原因：未知。", "Power Systems Nominal", 'sound/AI/poweron.ogg'), 10 SECONDS)
